...
class BlogPostGenerator {
    ...
    private companion object {
        fun List<String>.concat() = this.reduce { s1, s2 -> s1 + s2 }
    ...
    fun generateBlogPosts() {
        ...
        val files = sourceDir
                ...
                .map { markdownToBlogEntry(it) }
                .map { toBlogEntryHtmlPair(it) }
        ...
        val indexHtml = Jsoup.parse(
                blogSourceIndexFile.readLines(Charset.forName("UTF-8")).concat()
        )
    ...
    private fun markdownToBlogEntry(it: String) = BlogEntryParser.instance
    ...
    private fun toBlogEntryHtmlPair(it: BlogEntry): Pair<BlogEntry, Document> =
            Pair(
                    it,
